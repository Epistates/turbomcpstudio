...
mod credential_redaction_tests {
    ...
    fn test_redact_sensitive_api_key() {
        ...
        assert!(!redacted.contains("abc123"), "Original value should not be visible");
        assert!(!redacted.contains("xyz789"), "Original value should not be visible");
    }
    ...
    fn test_redact_short_password() {
        ...
        assert!(!redacted.contains("abc"), "Short password should be fully hidden");
    }
    ...
    fn test_redact_various_sensitive_patterns() {
        ...
        for (key, value) in test_cases {
            let redacted = redact_test_value(key, value);
            assert!(redacted.contains("REDACTED"), "Key {} should be redacted", key);
