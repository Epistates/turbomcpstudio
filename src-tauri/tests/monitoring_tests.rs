...
fn test_arc_dashmap_sharing() {
    ...
    assert_eq!(
        ...
        1,
        "Monitoring sees initial connection"
    );
    ...
    assert!(
        monitoring_connections.contains_key(&id1),
        "Should see connection 1"
    );
    assert!(
        monitoring_connections.contains_key(&id2),
        "Should see connection 2"
    );
}
...
fn test_broken_snapshot_pattern() {
    ...
    assert_eq!(
        ...
        1,
        "Snapshot has initial connection"
    );
    ...
    assert_eq!(
        ...
        2,
        "But manager has 2 connections"
    );

    // Monitoring never sees the new connection
    assert!(
        monitoring_snapshot.contains_key(&id1),
        "Snapshot has connection 1"
    );
    assert!(
        !monitoring_snapshot.contains_key(&id2),
        "Snapshot MISSING connection 2!"
    );
}
...
fn test_connection_removal_visible_to_monitoring() {
    ...
    assert_eq!(
        monitoring_connections.len(),
        1,
        "Monitoring should see connection removal"
    );
    assert!(
        !monitoring_connections.contains_key(&id1),
        "Removed connection should be gone"
    );
    assert!(
        monitoring_connections.contains_key(&id2),
...
fn test_arc_reference_counting() {
    ...
    assert_eq!(
        Arc::strong_count(&manager.connections),
        1,
        "Only manager reference remains"
    );
}
...
fn test_connection_metadata_sharing() {
    ...
    assert!(
        ...
        "Should be the same Arc<MockConnection>"
    );
}
...
fn test_monitoring_pattern_correctness() {
    ...
    for (i, id) in server_ids.iter().enumerate() {
        assert!(
            ...
            "Connection {} should be visible to monitoring",
            i
        );
    }
    ...
    assert_eq!(
        monitoring_connections.len(),
        2,
        "Monitoring should see disconnection"
    );
    assert!(
        !monitoring_connections.contains_key(&server_ids[1]),
        "Disconnected server should be gone"
    );
}
...
fn test_empty_monitoring_initialization() {
    ...
    assert_eq!(
        monitoring.len(),
        0,
        "Monitoring can start with empty DashMap"
    );
