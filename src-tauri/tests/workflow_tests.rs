...
async fn test_workflow_state_sharing() {
    ...
    // Simulate command 2: get_workflow_execution (different command, same engine)
    {
        ...
        assert!(
            execution.is_some(),
            "Different command should see the same execution state"
        );
    ...
    // Simulate command 3: stop_workflow_execution (yet another command, same engine)
    {
        ...
        assert!(
            stopped,
            "Should be able to stop execution from different command"
        );
    }
...
async fn test_workflow_state_isolation_without_arc() {
    ...
    assert!(
        !stopped,
        "Cannot stop execution that doesn't exist in this engine instance"
    );
}
...
async fn test_cooperative_cancellation_checks() {
    ...
    for step in 1..=5 {
        ...
        if engine.is_cancelled(execution_id).await {
            ...
            assert!(
                step > 1,
                "Should have executed at least one step before cancelling"
            );
...
async fn test_cancellation_flag_behavior() {
    ...
    assert!(
        !engine.is_cancelled(execution_id).await,
    ...
    assert!(
        engine.is_cancelled(execution_id).await,
        "Should be cancelled after stop"
    );
}
...
async fn test_multiple_concurrent_executions() {
    ...
    assert!(
        engine.get_execution(id1).await.is_some(),
        "Execution 1 should exist"
    );
    assert!(
        engine.get_execution(id2).await.is_some(),
        "Execution 2 should exist"
    );
    assert!(
        engine.get_execution(id3).await.is_some(),
        "Execution 3 should exist"
    );
    ...
    assert!(
        !engine.is_cancelled(id1).await,
        "Execution 1 should not be cancelled"
    );
    assert!(
        engine.is_cancelled(id2).await,
        "Execution 2 should be cancelled"
    );
    assert!(
        !engine.is_cancelled(id3).await,
        "Execution 3 should not be cancelled"
    );
}
...
async fn test_workflow_status_transitions() {
    ...
    // Start: None -> Running
    assert!(
        engine.get_execution(execution_id).await.is_none(),
...
async fn test_nonexistent_execution_handling() {
    ...
    assert!(
        engine.get_execution(fake_id).await.is_none(),
    ...
    assert!(
        !stopped,
        "Stopping non-existent execution should return false"
    );

    // Check cancellation of non-existent execution
    assert!(
        !engine.is_cancelled(fake_id).await,
...
async fn test_arc_clone_behavior() {
    ...
    assert!(
        engine2.get_execution(execution_id).await.is_some(),
        "Arc clone should reference same data"
    );

    // Verify Arc strong count
    assert_eq!(
        Arc::strong_count(&engine1),
        2,
        "Should have 2 strong references"
    );
}
