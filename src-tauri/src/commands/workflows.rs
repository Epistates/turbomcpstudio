...
pub async fn get_workflow_execution(
    ...
    // First check if the execution is currently active using shared engine
    if let Ok(Some(active_execution)) = app_state.workflow_engine.get_execution(uuid).await {
